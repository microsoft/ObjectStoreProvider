...
export type DBStore<Name extends string, ObjectType, KeyFormat> = string & {
  name?: Name;
...
> {
  constructor(
    ...
    _storeName: /* Force type-checking */ DBStore<
      StoreName,
      ObjectType,
      KeyFormat
    >,
  ...
  getAll(sortOrder?: QuerySortOrder): Promise<ObjectType[]> {
    let promise = this._store.openPrimaryKey().getAll(sortOrder) as Promise<
      ObjectType[]
    >;
    return ErrorCatcher ? promise.catch(ErrorCatcher) : promise;
  }
  ...
  getOnly(
    ...
    let promise = this._store
      .openPrimaryKey()
      .getOnly(key, reverseOrSortOrder, limit, offset) as Promise<ObjectType[]>;
    return ErrorCatcher ? promise.catch(ErrorCatcher) : promise;
  }

  getRange(
    keyLowRange: KeyFormat,
    keyHighRange: KeyFormat,
    lowRangeExclusive?: boolean,
    highRangeExclusive?: boolean,
    reverseOrSortOrder?: boolean | QuerySortOrder,
    limit?: number,
  ...
  openIndex<IndexKeyFormat extends KeyType>(
    indexName: DBIndex<
