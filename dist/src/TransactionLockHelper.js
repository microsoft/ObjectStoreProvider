...
exports.TransactionLockHelper = exports.Deferred = void 0;
/**
...
var TransactionLockHelper = /** @class */ (function () {
    ...
    TransactionLockHelper.prototype.openTransaction = function (storeNames, exclusive) {
        ...
        if (storeNames) {
            ...
            if (missingStore) {
                return Promise.reject("Opened a transaction with a store name (" +
                    missingStore +
    ...
    TransactionLockHelper.prototype.transactionComplete = function (token) {
        ...
        if (pendingTransIndex !== -1) {
            ...
            if (pendingTrans.completionDefer) {
                ...
                throw new Error("Completing a transaction that has already been completed. Stores: " +
            ...
            throw new Error("Completing a transaction that is no longer tracked. Stores: " +
                token.storeNames.join(","));
        }
    ...
    TransactionLockHelper.prototype.transactionFailed = function (token, message) {
        ...
        if (pendingTransIndex !== -1) {
            ...
            throw new Error("Failing a transaction that is no longer tracked. Stores: " +
                token.storeNames.join(",") +
                ", message: " +
                message);
        }
    ...
    TransactionLockHelper.prototype._cleanTransaction = function (token) {
        ...
        if (token.exclusive) {
            lodash_1.each(token.storeNames, function (storeName) {
                if (!_this._exclusiveLocks[storeName]) {
                    throw new Error("Missing expected exclusive lock for store: " + storeName);
                }
            ...
            lodash_1.each(token.storeNames, function (storeName) {
                if (!(_this._readOnlyCounts[storeName] > 0)) {
                    throw new Error("Missing expected readonly lock for store: " + storeName);
                }
    ...
    TransactionLockHelper.prototype._checkNextTransactions = function () {
        var _this = this;
        if (lodash_1.some(this._exclusiveLocks, function (lock) { return lock; }) &&
        ...
        var _loop_1 = function (i) {
            ...
            if (lodash_1.some(trans.token.storeNames, function (storeName) {
                return _this._exclusiveLocks[storeName] ||
