...
var IndexedDbProvider = /** @class */ (function (_super) {
    ...
    function IndexedDbProvider(explicitDbFactory, explicitDbFactorySupportsCompoundKeys, handleOnClose) {
        ...
        if (explicitDbFactory) {
            ...
            if (typeof explicitDbFactorySupportsCompoundKeys !== "undefined") {
                ...
                _this._fakeComplicatedKeys = ObjectStoreProviderUtils_1.isIE();
            }
        }
        if (handleOnClose) {
    ...
    IndexedDbProvider.prototype.open = function (dbName, schema, wipeIfExists, verbose) {
        return __awaiter(this, void 0, void 0, function () {
            var req, e_1, dbOpen, migrationPutters, promise;
            var _this = this;
                ...
                switch (_a.label) {
                    case 0:
                        ...
                        if (!this._dbFactory) {
                            ...
                            return [2 /*return*/, Promise.reject("No support for IndexedDB in this browser")];
                        }
                        if (!wipeIfExists) return [3 /*break*/, 4];
                        _a.label = 1;
                    case 1:
                        _a.trys.push([1, 3, , 4]);
                        req = this._dbFactory.deleteDatabase(dbName);
                        return [4 /*yield*/, IndexedDbProvider.WrapRequest(req)];
                    case 2:
                        _a.sent();
                        return [3 /*break*/, 4];
                    case 3:
                    ...
                    case 4:
                        ...
                        dbOpen = this._dbFactory.open(dbName, schema.version);
                        migrationPutters = [];
                        dbOpen.onupgradeneeded = function (event) {
                            ...
                            lodash_1.each(schema.stores, function (storeSchema) {
                                ...
                                if (!storeExistedBefore) {
                                    ...
                                    if (_this._fakeComplicatedKeys && ObjectStoreProviderUtils_1.isCompoundKeyPath(primaryKeyPath)) {
                                    ...
                                    if (!lodash_1.includes(store.indexNames, indexSchema.name)) {
                                        ...
                                        if (_this._fakeComplicatedKeys) {
                                            if (indexSchema.multiEntry || indexSchema.fullText) {
                                                if (ObjectStoreProviderUtils_1.isCompoundKeyPath(keyPath)) {
                                            ...
                                            else if (ObjectStoreProviderUtils_1.isCompoundKeyPath(keyPath)) {
                        ...
                        promise = IndexedDbProvider.WrapRequest(dbOpen);
                        return [2 /*return*/, promise.then(function (db) {
                                ...
                                if (err &&
                                    ...
                                    if (!wipeIfExists) {
                                        console.log("Database version too new, Wiping: " +
...
var IndexedDbTransaction = /** @class */ (function () {
    function IndexedDbTransaction(_trans, lockHelper, _transToken, _schema, _fakeComplicatedKeys) {
        ...
        this._stores = lodash_1.map(this._transToken.storeNames, function (storeName) {
            ...
            this._trans.onerror = function () {
                ...
                if (history_1.length > 1) {
                    console.warn("IndexedDbTransaction Errored after Resolution, Swallowing. Error: " +
                        (_this._trans.error ? _this._trans.error.message : undefined) +
                        ", History: " +
                        history_1.join(","));
                    return;
                }
...
var IndexedDbStore = /** @class */ (function () {
    ...
    IndexedDbStore.prototype.get = function (key) {
        var _this = this;
        if (this._fakeComplicatedKeys &&
            ObjectStoreProviderUtils_1.isCompoundKeyPath(this._schema.primaryKeyPath)) {
            var err = lodash_1.attempt(function () {
                key = ObjectStoreProviderUtils_1.serializeKeyToString(key, _this._schema.primaryKeyPath);
            });
            if (err) {
                return Promise.reject(err);
            }
    ...
    IndexedDbStore.prototype.getMultiple = function (keyOrKeys) {
        ...
        var keys = lodash_1.attempt(function () {
            var keys = ObjectStoreProviderUtils_1.formListOfKeys(keyOrKeys, _this._schema.primaryKeyPath);
            if (_this._fakeComplicatedKeys &&
                ObjectStoreProviderUtils_1.isCompoundKeyPath(_this._schema.primaryKeyPath)) {
                return lodash_1.map(keys, function (key) {
                    return ObjectStoreProviderUtils_1.serializeKeyToString(key, _this._schema.primaryKeyPath);
                });
            }
            return keys;
        });
        ...
        var err = lodash_1.attempt(function () {
            lodash_1.each(items, function (item) {
                ...
                if (_this._fakeComplicatedKeys) {
                    ...
                    if (ObjectStoreProviderUtils_1.isCompoundKeyPath(_this._schema.primaryKeyPath)) {
                        fakedPk = true;
                        item["nsp_pk"] = ObjectStoreProviderUtils_1.getSerializedKeyForKeypath(item, _this._schema.primaryKeyPath);
                    }
                    lodash_1.each(_this._schema.indexes, function (index) {
                        if (index.multiEntry || index.fullText) {
                            ...
                            if (index.fullText) {
                                ...
                                var valsRaw = ObjectStoreProviderUtils_1.getValueForSingleKeypath(item, index.keyPath);
                                // It might be an array of multiple entries, so just always go with array-based logic
                                keys_1 = ObjectStoreProviderUtils_1.arrayify(valsRaw);
                            }
                            var refKey_1;
                            var err_1 = lodash_1.attempt(function () {
                                ...
                                if (ObjectStoreProviderUtils_1.isCompoundKeyPath(index.keyPath)) {
                                    keys_1 = lodash_1.map(keys_1, function (val) {
                                        return ObjectStoreProviderUtils_1.serializeKeyToString(val, index.keyPath);
                                    });
                                }
                                ...
                                // raw value.
                                refKey_1 = ObjectStoreProviderUtils_1.getKeyForKeypath(item, _this._schema.primaryKeyPath);
                                if (lodash_1.isArray(_this._schema.primaryKeyPath)) {
                                    refKey_1 = ObjectStoreProviderUtils_1.serializeKeyToString(refKey_1, _this._schema.primaryKeyPath);
                                }
                            });
                            if (err_1) {
                                errToReport = err_1;
                            ...
                            var cursorReq = indexStore_1
                                .index("refkey")
                                .openCursor(IDBKeyRange.only(refKey_1));
                            promises.push(IndexedDbIndex.iterateOverCursorRequest(cursorReq, function (cursor) {
                                ...
                                var iputters = lodash_1.map(keys_1, function (key) {
                                    ...
                                    return IndexedDbProvider.WrapRequest(indexStore_1.put(indexObj)).then(function () { return void 0; });
                                });
                                return Promise.all(iputters);
                            })
                                .then(lodash_1.noop));
                        }
                        else if (ObjectStoreProviderUtils_1.isCompoundKeyPath(index.keyPath)) {
                            item[IndexPrefix + index.name] =
                                ObjectStoreProviderUtils_1.getSerializedKeyForKeypath(item, index.keyPath);
                        }
                        return true;
                    });
                }
                ...
                if (!errToReport) {
                    errToReport = lodash_1.attempt(function () {
                        ...
                        promises.push(IndexedDbProvider.WrapRequest(req).then(function () { return void 0; }));
                    });
                }
                if (errToReport) {
    ...
    IndexedDbStore.prototype.remove = function (keyOrKeys) {
        ...
        return Promise.all(lodash_1.map(keys, function (key) {
            if (_this._fakeComplicatedKeys &&
                lodash_1.some(_this._schema.indexes, function (index) { return index.multiEntry || index.fullText; })) {
                ...
                return IndexedDbProvider.WrapRequest(_this._store.get(key)).then(function (item) {
                    if (item) {
                        ...
                        var promises = lodash_1.map(lodash_1.filter(_this._schema.indexes, function (index) { return !!index.multiEntry; }), function (index) {
                            var indexStore = lodash_1.find(_this._indexStores, function (store) {
                            ...
                            var refKey = lodash_1.attempt(function () {
                                ...
                                // compound, we're already faking complicated keys, so we know to serialize it to a string.  If not, use the
                                // raw value.
                                var tempRefKey = ObjectStoreProviderUtils_1.getKeyForKeypath(item, _this._schema.primaryKeyPath);
                                return lodash_1.isArray(_this._schema.primaryKeyPath)
                                    ? ObjectStoreProviderUtils_1.serializeKeyToString(tempRefKey, _this._schema.primaryKeyPath)
                                    : tempRefKey;
                            });
    ...
    IndexedDbStore.prototype.openIndex = function (indexName) {
        ...
        if (this._fakeComplicatedKeys &&
            (indexSchema.multiEntry || indexSchema.fullText)) {
            var store = lodash_1.find(this._indexStores, function (indexStore) {
    ...
    IndexedDbStore.prototype.clearAllData = function () {
        ...
        var promises = lodash_1.map(storesToClear, function (store) {
...
var IndexedDbIndex = /** @class */ (function (_super) {
    ...
    IndexedDbIndex.prototype._resolveCursorResult = function (req, limit, offset) {
        ...
        if (this._fakeComplicatedKeys && this._fakedOriginalStore) {
            ...
            return IndexedDbIndex.getFromCursorRequest(req, limit, offset).then(function (rets) {
                ...
                var getters = lodash_1.map(rets, function (ret) {
    ...
    IndexedDbIndex.prototype.getMultiple = function (keyOrKeys) {
        var _this = this;
        var keys = lodash_1.attempt(function () {
            var keys = ObjectStoreProviderUtils_1.formListOfKeys(keyOrKeys, _this._keyPath);
            return keys;
        });
        if (lodash_1.isError(keys)) {
            return Promise.reject(keys);
        ...
        return Promise.all(lodash_1.map(keys, function (key) { return _this.getOnly(key); })).then(function (vals) {
            return lodash_1.compact(lodash_1.flatten(vals));
        });
    };
    // Warning: This function can throw, make sure to trap.
    IndexedDbIndex.prototype._getKeyRangeForOnly = function (key) {
        if (this._fakeComplicatedKeys && ObjectStoreProviderUtils_1.isCompoundKeyPath(this._keyPath)) {
            return IDBKeyRange.only(ObjectStoreProviderUtils_1.serializeKeyToString(key, this._keyPath));
        }
    ...
    IndexedDbIndex.prototype._getKeyRangeForRange = function (keyLowRange, keyHighRange, lowRangeExclusive, highRangeExclusive) {
        if (this._fakeComplicatedKeys && ObjectStoreProviderUtils_1.isCompoundKeyPath(this._keyPath)) {
            ...
            return IDBKeyRange.bound(ObjectStoreProviderUtils_1.serializeKeyToString(keyLowRange, this._keyPath), ObjectStoreProviderUtils_1.serializeKeyToString(keyHighRange, this._keyPath), lowRangeExclusive, highRangeExclusive);
        }
