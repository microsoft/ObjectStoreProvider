...
exports.DbIndexFTSFromRangeQueries = exports.getFullTextIndexWordsForItem = exports.breakAndNormalizeSearchPhrase = void 0;
...
function breakAndNormalizeSearchPhrase(phrase) {
    ...
    var uniqueWordas = lodash_1.map(lodash_1.mapKeys(wordInPhrase), function (_value, key) {
...
function getFullTextIndexWordsForItem(keyPath, item) {
    var rawString = ObjectStoreProviderUtils_1.getValueForSingleKeypath(item, keyPath);
...
var DbIndexFTSFromRangeQueries = /** @class */ (function () {
    ...
    DbIndexFTSFromRangeQueries.prototype.fullTextSearch = function (searchPhrase, resolution, limit) {
        var _this = this;
        if (resolution === void 0) { resolution = ObjectStoreProvider_1.FullTextTermResolution.And; }
        ...
        var promises = lodash_1.map(terms, function (term) {
            var upperEnd = term.substr(0, term.length - 1) +
                String.fromCharCode(term.charCodeAt(term.length - 1) + 1);
            return _this.getRange(term, upperEnd, false, true, false, limit);
        });
        return Promise.all(promises).then(function (results) {
            var uniquers = lodash_1.attempt(function () {
                return lodash_1.map(results, function (resultSet) {
                    return lodash_1.keyBy(resultSet, function (item) {
                        return ObjectStoreProviderUtils_1.getSerializedKeyForKeypath(item, _this._primaryKeyPath);
                    });
                });
            });
            ...
            if (resolution === ObjectStoreProvider_1.FullTextTermResolution.Or) {
                var data = lodash_1.values(lodash_1.assign.apply(void 0, __spreadArray([{}], __read(uniquers))));
                if (limit) {
                    return lodash_1.take(data, limit);
                }
                return Promise.resolve(data);
            }
            if (resolution === ObjectStoreProvider_1.FullTextTermResolution.And) {
                var _a = __read(uniquers), first = _a[0], others_1 = _a.slice(1);
                var dic = lodash_1.pickBy(first, function (_value, key) {
                    return lodash_1.every(others_1, function (set) { return key in set; });
                });
